...
from amr_mapping.map_store_py import *
...
class SonarMap:
    ...
    def add_scan(self, m_sonar_x, m_sonar_y, sonar_theta, field_of_view,
        ...
        if (self._convert_to_cell([m_sonar_x,m_sonar_y]) != None):
            #convert sonar map coordinates to cell coordinates
            ...
            #create cone using MapStoreCone function
            cone = MapStoreCone(c_sonar_pos_x, c_sonar_pos_y, sonar_theta,
                                field_of_view, c_cone_length)

            #Iterate each cell in the cone
            temp = 0
            for cell in cone:
                cell_map = self._convert_to_map(cell)
                if(cell_map != None):
                    #calculate euclidian distance between sonar and each cell
                    distance = self.euclidian_distance(m_sonar_x,m_sonar_y,cell_map[0],cell_map[1])
                    
                    #calculate angle between main axis of sensor and line S to P
                    adjacent = cell_map[0] - m_sonar_x
                    opposite = cell_map[1] - m_sonar_y
                    temp_theta = math.atan2(opposite,adjacent)
                    theta = self.angular_distance(temp_theta,sonar_theta)

                    #calculate angular probability of each cell
                    angular_probability = self._ea(field_of_view, theta)
                    
                    #calculate distance probability for free space 
                    distance_probability_free_space = self._er_free(registerd_range, distance, uncertainty)
                    #calculate distance probability for occupied space
                    distance_probability_occupied_space = self._er_occ(registerd_range, distance, uncertainty)
                    
                    #compute probability of free space
                    probability_free = distance_probability_free_space * angular_probability
                    #compute probability of occupied space
                    probability_occupied = distance_probability_occupied_space * angular_probability
                    
                    #calculate set of free space
                    emp_x_y = self._map_free.get(cell[0],cell[1]) + (1 - self._map_free.get(cell[0],cell[1])) *  probability_free
                    emp_x_y = self.clamp(emp_x_y,0,1)
                    self._map_free.set(cell[0],cell[1],emp_x_y)

                    #calculate set of occupied space
                    if(max_range != registerd_range):
                        occ_x_y_c = probability_occupied * (1-emp_x_y)
                        temp += occ_x_y_c 
                        ...
                        if(temp == 0):
                            ...
                            occ_x_y_n = occ_x_y_c / temp

                        occ_x_y_t = self._map_occupied.get(cell[0],cell[1]) + occ_x_y_n - (self._map_occupied.get(cell[0],cell[1]) * occ_x_y_n)
                        occ_x_y_t = self.clamp(occ_x_y_t,0,1)
                        self._map_occupied.set(cell[0],cell[1],occ_x_y_t)
                    else:
                        occ_x_y_t = 0
                        self._map_occupied.set(cell[0],cell[1],occ_x_y_t)
                    
                    #calculate combined probability
                    if(occ_x_y_t > emp_x_y):
                        self._map_combined.set(cell[0],cell[1],occ_x_y_t)
                    elif(occ_x_y_t < emp_x_y):
                        self._map_combined.set(cell[0],cell[1],-emp_x_y)
                    else:
                        self._map_combined.set(cell[0],cell[1],0)
            pass
    ...
    def _er_free(self, sensed_distance, delta, uncertainty):
        if(delta < (sensed_distance-uncertainty)):
    ...
    def _er_occ(self, sensed_distance, delta, uncertainty):
        if((sensed_distance-uncertainty) <= delta and (sensed_distance + uncertainty) >= delta):
            return(1-pow((delta-sensed_distance)/uncertainty,2))
        else:
            return 0.0
        """
    ...
    def _convert_to_cell(self, m_pos):
        ...
        """
        m_x, m_y = m_pos
        #rospy.logwarn("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx  "+str(m_x))
